from pde_solver.Fourier import Fourier_coef, Fourier_serie
from pde_solver.create_database import diffusion_1D, wave_1D
...
def get_data(cfact=[1], T=[1], u0_param=[1]):
    ...
    solutions = np.zeros(shape=(nbr_sol, 2, max_len))
    ...
    for u0_par in u0_param:
        ...
        for c_d, dt_d, dx_d, c_w, dt_w, dx_w in zip(c_diff, dt_diff, dx_diff, c_wave, dt_wave, dx_wave):
            for t in T:
                ...
                solutions[index] = diffs
                solutions[index+1] = waves
                labels[index] = 0
                labels[index+1] = 1
                index += 2
        n+=1
